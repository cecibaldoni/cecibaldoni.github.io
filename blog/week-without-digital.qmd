...
bridge <- data.frame(day = 7.5, period = factor("Before", levels = c("Before", "During")), value = NA,
                     metric = "Words per day")
notebook_long <- notebook_data %>%
  pivot_longer(cols = c(doodles, words, bullets), names_to = "metric", values_to = "value") %>%
  mutate(metric = factor(metric,
                         levels = c("words", "doodles", "bullets"),
                         labels = c("Words per day", "Doodles per day", "Bullet points per day")))

ggplot(notebook_long |> filter(metric == "Words per day") |> bind_rows(bridge) |> arrange(day), aes(x = day, y = value, group = metric)) +
  annotate("rect", xmin = 7.5, xmax = 14.5, ymin = -Inf, ymax = Inf, fill = "#f5ede4", alpha = 0.5) +
  geom_line(color = "grey80", linewidth = 2) +
  geom_line(aes(color = period), linewidth = 2) +
  geom_point(aes(color = period), size = 4) +
  scale_color_manual(values = palette) +
  scale_x_continuous(breaks = 1:14, labels = c(paste0("D", 1:7), paste0("D", 1:7))) +
  labs(title = "Words per day", x = "Day", y = "Word Count") +
  base_theme +
  plot_annotation(caption  = "Peer review status: pending indefinitely.")
```
...
ggplot(notebook_data, aes(x = period, y = margin_pct, fill = period)) +
...
diverging_data <- notebook_long %>%
  group_by(metric, period) %>%
  summarise(mean_value = mean(value), .groups = "drop") %>%
  mutate(mean_diverging = ifelse(period == "Before", -mean_value, mean_value))


diverging_data_small <- notebook_long %>%
  filter(metric %in% c("Doodles per day", "Bullet points per day")) %>%
  group_by(metric, period) %>%
  summarise(mean_value = mean(value), .groups = "drop") %>%
  mutate(mean_diverging = ifelse(period == "Before", -mean_value, mean_value))

ggplot(diverging_data_small, aes(x = metric, y = mean_diverging, fill = period)) +
  geom_col(width = 0.5, alpha = 0.9) +
  geom_hline(yintercept = 0, color = "grey30", linewidth = 0.6) +
  geom_text(data = diverging_data_small %>% filter(period == "Before"),
            aes(label = round(abs(mean_diverging), 1)),
            vjust = 1.3, size = 3, color = "grey30") +

  geom_text(data = diverging_data_small %>% filter(period == "During"),
            aes(label = round(abs(mean_diverging), 1)),
            vjust = -0.3, size = 3, color = "grey30") +

  scale_fill_manual(values = palette) +
  scale_y_continuous(labels = abs) +
  labs(title = "Doodles and Bullet Points", x = NULL, y = "Mean") +
  base_theme +
